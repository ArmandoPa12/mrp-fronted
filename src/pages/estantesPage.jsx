...
const fetchCharacter = async () => {
    try {
      ...
      setFilas(response.data.estantes);
      console.log(response.data.estantes);
    } catch (error) {
    ...
    const saveProduct =async () => {
        ...
        if (fila.descripcion.trim()) {
            ...
            if(fila.id){
                try {
                    await axios.put(`http://127.0.0.1:8000/api/${direccion}/${fila.id}`,fila,cabeza);
                    setProductDialog(false);
                    setFila(emptyUsuario);
                    fetchCharacter();
                } catch (error) {
                ...
                try {
                    await axios.post(`http://127.0.0.1:8000/api/${direccion}`,fila,cabeza);
                    setProductDialog(false);
                    setFila(emptyUsuario);
                    fetchCharacter();
                } catch (error) {
    ...
    const hideDeleteProductsDialog = () => {
      ...
      const deleteProduct = async () => {
        try {
            await axios.delete(`http://127.0.0.1:8000/api/${direccion}/${fila.id}`,cabeza);
            fetchCharacter();
            setDeleteProductsDialog(false);
            setFila(emptyUsuario);
        } catch (error) {
        ...
        {permiso(17)?(
            ...
            <Button label="New" icon="pi pi-plus" severity="success" onClick={openNew} />
                ...
                >
                    ...
                    {fila && (
                        <span>
                        Seguro que quieres eliminar este estante? <b>{fila.codigo}</b>?
                        </span>
                    )}
                    </div>
                </Dialog>
